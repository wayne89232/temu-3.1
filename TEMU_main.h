...
#define PROP_MODE_XFORM	1
  ...
  void (*read_disk_taint)(int size, int64_t sect_num, int flag,uint32_t paddr);
  ...
  void (*after_iret_protected)(void);
#endif
...
void TEMU_virtdev_init(void);
void TEMU_after_loadvm(void);
void TEMU_init(void);
#ifdef DEFINE_BLOCK_BEGIN
int TEMU_block_begin(void);
#endif
#ifdef DEFINE_INSN_BEGIN
void TEMU_insn_begin(uint32_t pc_start);
#endif
#ifdef DEFINE_INSN_END
void TEMU_insn_end(void);
#endif
void TEMU_update_cr3(void);
void TEMU_do_interrupt(int intno, int is_int, target_ulong next_eip);
void TEMU_after_iret_protected(void);
void TEMU_update_cpustate(void);
